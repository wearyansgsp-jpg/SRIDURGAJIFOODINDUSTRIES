...
function AuthPage() {
  ...
  return (
    <div className="min-h-screen grid place-items-center bg-gradient-hero p-4 text-cream">
      <form onSubmit={onSubmit} className="w-full max-w-md rounded-lg bg-white p-8 text-charcoal shadow-soft">
        ...
        <label className="mt-6 block text-xs font-semibold uppercase tracking-widest text-brown">Email</label>
        <input
          type="email" required value={email} onChange={(e) => setEmail(e.target.value)}
          className="mt-2 w-full rounded-md border border-input bg-background px-4 py-3 text-sm outline-none focus:border-gold focus:ring-2 focus:ring-gold/30"
          autoComplete="email"
        />

        <label className="mt-4 block text-xs font-semibold uppercase tracking-widest text-brown">Password</label>
        <input
